  ...
  if(form.checkValidity()){
    ...
    .then(result => {
      if(result.status === 200 || result.status === 201){
        ...
        allUsers.push(result.info); // Agregar nuevo usuario a la lista global
        renderUsers(allUsers); // Volver a renderizar tabla
      } else {
        document.getElementById('info').innerHTML = 
          '<h3 class="text-danger">No se guardó el usuario en la API <i class="fa-solid fa-x"></i></h3>';
      }
